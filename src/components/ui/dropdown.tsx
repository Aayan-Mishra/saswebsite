    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className={`absolute z-[9999] min-w-[14rem] overflow-hidden rounded-xl border border-border bg-white p-1.5 shadow-lg shadow-navy-900/10 ${positionClasses} ${alignmentClasses} ${className}`}
          role="menu"
...
export function DropdownItem({
  ...
  return (
    <button
      className={`relative flex w-full cursor-pointer select-none items-center rounded-lg px-3 py-2.5 text-sm outline-none transition-colors ${
        disabled
...
export function DropdownSeparator({ className = "" }: DropdownSeparatorProps) {
  return (
    <div className={`mx-1 my-1 h-px bg-navy-100 ${className}`} role="separator" />
  )
}
